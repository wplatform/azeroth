...
{
    Id = spellEntry->Id;
    SpellMiscEntry const* spellMisc = sSpellMiscStore.LookupEntry(spellEntry->MiscId);
    ...
    for (uint8 i = 0; i < 2; ++i)
        SpellVisual[i] = spellMisc ? spellMisc->SpellVisualID[i] : 0;
...
{
    return Id ? sSpellPowerStore.LookupEntry(Id) : nullptr;
}
